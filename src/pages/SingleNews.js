  ...
  return (
    <div>
      ...
      <Hero>
        <div className="row single-news-hero">
          <div className="col-12">
            <h1 className="hero-title">{thisNews.title}</h1>
            <div className="article-meta hero-title">
              <p>
                by Admin <span>|</span>
              </p>
              <p>
                0 comments <span>|</span>
              </p>
              <p>{thisNews.date}</p>
            </div>
            <ul className="hero-title">
              {thisNews.singlePage.categories.map((item, index) => (
                <li key={index}>
                  <a href="/#">{item}</a>
                </li>
              ))}
        ...
        <ArticlesDetails>
          <div className="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-8 single-news-left">
            <h1 className="section-big-title anim-bot">
              {thisNews.singlePage.headline}
            </h1>
            {thisNews.singlePage.textOne.map((item, i) => (
              <p className="anim-bot" key={i}>
                {item}
              </p>
            ))}
            {thisNews.singlePage.blockquote.text ? (
              <blockquote className="anim-bot">
                <p>{thisNews.singlePage.blockquote.text}</p>
                <cite>{thisNews.singlePage.blockquote.author}</cite>
              </blockquote>
            ) : null}
            {thisNews.singlePage.textTwo.map((item, i) => (
              <p className="anim-bot" key={i}>
                {item}
              </p>
            ))}
            <p className="tags anim-bot">
              {thisNews.singlePage.tags.map((item, i) => (
      ...
      <ArticlesNav
        backgroundPrev={
          thisNews.id > 1
            ? articlesData.items.find(
                (element) => element.id === thisNews.id - 1
              ).image
            : thisNews.image
        }
        titlePrev={
        ...
        backgroundNext={
          thisNews.id < articlesData.items.length
            ? articlesData.items.find(
                (element) => element.id === thisNews.id + 1
              ).image
            : thisNews.image
